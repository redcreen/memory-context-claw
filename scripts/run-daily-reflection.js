...
import { renderDailyReflectionReport } from "../src/unified-memory-core/index.js";
...
const report = await runtime.runDailyReflection({
  declaredSources: [
    {
      ...
      declaredBy: readFlag("--declared-by", "daily-reflection-script"),
      namespace: {
        tenant: readFlag("--tenant", "local"),
        scope: readFlag("--scope", "workspace"),
        resource: readFlag("--resource", "unified-memory-core"),
  ...
  autoPromote: readBooleanFlag("--promote")
});

if (readFlag("--format", "json") === "markdown") {
  console.log(renderDailyReflectionReport(report));
} else {
