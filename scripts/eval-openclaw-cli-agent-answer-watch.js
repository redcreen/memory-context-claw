...
const defaultWatchCaseIds = [
  "agent-name-1",
  "agent-project-1",
  "agent-rule-no-guess-1",
  "agent-cross-source-calls-1",
  "agent-current-editor-1",
  "agent-current-demo-1",
  ...
  "agent-zh-natural-history-editor-1",
  "agent-zh-natural-conflict-region-1",
  "agent-zh-natural-cross-source-calls-1",
...
const args = [
  ...
  ...(!hasFlag("--only") ? ["--only", defaultWatchCaseIds.join(",")] : []),
  ...(!hasFlag("--write-json")
    ? ["--write-json", path.resolve(__dirname, `../reports/openclaw-cli-agent-answer-watch-${date}.json`)]
    : []),
  ...(!hasFlag("--write-markdown")
    ? ["--write-markdown", path.resolve(__dirname, `../reports/generated/openclaw-cli-agent-answer-watch-${date}.md`)]
    : []),
  ...forwarded
];
