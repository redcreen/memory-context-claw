...
test("buildAssemblyResult prefers a dedicated family overview card for children overview queries", () => {
  const result = buildAssemblyResult({
    ...
    candidates: [
      {
        ...
        weightedScore: 1.12,
        finalScore: 1.12,
        snippet: "你有一儿一女，孩子情况是：女儿叫刘子妍，生日是2014-12-29，现在上五年级；儿子叫刘子暄，生日是2007-07-25，现在上高三"
      },
      {
        path: "memory/2026-04-05.md",
        pathKind: "cardArtifact",
  ...
  assert.deepEqual(
    ...
    ["你有一儿一女，孩子情况是：女儿叫刘子妍，生日是2014-12-29，现在上五年级；儿子叫刘子暄，生日是2007-07-25，现在上高三"]
  );
});

test("buildAssemblyResult falls back to two stable fact cards from the same path for children overview queries", () => {
  const result = buildAssemblyResult({
    messages: [{ role: "user", content: "我家孩子的情况你记住了吗", timestamp: 1 }],
    tokenBudget: 2048,
    memoryBudgetRatio: 0.35,
