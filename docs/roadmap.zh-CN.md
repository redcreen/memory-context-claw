...
- 当前解释：Stage 7 的 operator-facing scorecard 和 Stage 9 的 guarded opt-in seam 都已落地，但真实 OpenClaw live soak 进一步证明宿主 runtime seam 仍未打通，所以 Stage 7 还没正式 closeout，Stage 9 也继续保持 `default-off` / opt-in only
...
| Stage 9：guarded smart-path promotion | `in_progress` | context 加载优化如果一直停在 shadow-only，就还不是用户收益 | bounded、guarded 的窄路径 experiment 有明确 rollback boundary、operator metrics 和 promotion gate，并继续保持 compat / compact 只是后台保底而不是默认日常路径；同时必须先补通 OpenClaw live runtime seam，才能让 guarded path 进入真实用户收益 |
...
| 时间层级 | 重点 | 退出信号 |
| --- | --- | --- |
| 当前 | 继续收 `Stage 7` closeout，并保持 `Stage 9` 为 default-off / opt-in only：先回答“怎样让每轮 context 更轻、更快、且不伤回答质量” | unified scorecard、guarded seam、rollback boundary 和 operator summary 都已落地，但 OpenClaw gateway live soak 证明宿主 runtime seam 仍是 blocker |
| 下一步 | 先补 OpenClaw live runtime seam，再继续收 `Stage 7` closeout，同时保持 `Stage 9` 的窄实验面；ordinary-conversation Docker steady-state 继续作为默认 hermetic A/B 基线 | unified scorecard、guarded seam、rollback boundary 和 operator summary 都已稳定，且剩余 blocker 已明确缩到 host-side decision seam 与 targeted Docker follow-up |
...
- 真实 OpenClaw live soak 已经证明：剩余 blocker 主要不是算法，而是宿主 runtime 没有把所需 subagent / decision seam 暴露给 context optimization 路径
