...
- 当前解释：Stage 7 的 operator-facing scorecard、harder live matrix、Stage 9 的 guarded live A/B，以及 Step 108 的 plugin-owned transport closeout 都已落地；当前主问题已经不再是宿主 seam，也不再是 guarded seam 能不能跑，而是转向 Stage 10 的 adoption simplification and shared-foundation proof
...
- 计划做：
  - 先把 `轻快` 里的 `Context Minor GC / context 加载优化` 做成下一轮主线，而不是继续把“安装体验”排在前面
...
1. `轻快 / context 加载优化`：先把每轮 context thickness、working-set reduction、budgeted assembly 和 answer-level latency 收成正式主线
   - 这条主线现在统一叫 `Context Minor GC`；目标不是更频繁 compact，而是尽量让长对话在日常路径上不用 compact 也能继续
...
| 时间层级 | 重点 | 退出信号 |
| --- | --- | --- |
| 当前 | Stage 7 已正式收口；转入 `Stage 10` 的 adoption simplification and shared-foundation proof | Context Minor GC unified scorecard、harder live matrix、Step 108 closeout 与 guarded seam boundary 都已齐备 |
| 下一步 | 先从 `121-123` 开始：缩短 install / bootstrap / verify，并把 package / startup / first-run 成本收进 `轻快` 证据面 | adoption 更短、更稳，同时不破坏 Stage 7/8/9 的证据面 |
| 更后面 | 在 `Stage 7` 站稳后，进入 `Stage 10` 的接入简化与跨宿主证明 | install / bootstrap / verify 与 shared-foundation proof 都变成 operator-ready 状态 |
