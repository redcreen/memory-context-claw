...
- OpenClaw gateway live validation：gateway service 已恢复、两轮 smoke 可用，但 `5 / 5` working-set exports 仍然全部报宿主 runtime seam 错误
- 当前解释：Stage 7 的 operator-facing scorecard 和 Stage 9 的 guarded opt-in seam 都已落地，但真实 OpenClaw live soak 进一步证明当前 `Context Minor GC` decision transport 仍绑在宿主 runtime seam 上；当前优先路线因此改为“插件内自托管 memory + context decision overlay”，而不是直接修改 OpenClaw
...
| Stage 9：guarded smart-path promotion | `in_progress` | context 加载优化如果一直停在 shadow-only，就还不是用户收益 | bounded、guarded 的窄路径 experiment 有明确 rollback boundary、operator metrics 和 promotion gate，并继续保持 compat / compact 只是后台保底而不是默认日常路径；优先先把 decision transport 收回插件内，再决定是否还需要宿主级 fallback |
...
| 时间层级 | 重点 | 退出信号 |
| --- | --- | --- |
| 当前 | 继续收 `Stage 7 / Context Minor GC` closeout，并保持 `Stage 9` 为 default-off / opt-in only：先回答“怎样让每轮 context 更轻、更快、且不伤回答质量” | unified scorecard、guarded seam、rollback boundary 和 operator summary 都已落地，但 OpenClaw gateway live soak 证明当前 decision transport 仍被宿主 runtime seam 卡住 |
| 下一步 | 优先实现插件内自托管 `memory + context decision overlay`，先把 `Context Minor GC` 的 working-set decision transport 从宿主 `subagent` seam 上解开，再继续收 `Stage 7` closeout；当前执行线已经明确成 4 步：`decision runner contract -> transport swap -> gateway live soak rerun -> Stage 7 closeout decision`；ordinary-conversation Docker steady-state 继续作为默认 hermetic A/B 基线 | unified scorecard、guarded seam、rollback boundary 和 operator summary 都已稳定，且剩余 blocker 已明确缩到 plugin-owned decision runner 与 targeted Docker follow-up |
