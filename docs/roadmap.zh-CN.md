...
- 当前解释：Stage 7 的 operator-facing scorecard 和 Stage 9 的 guarded opt-in seam 都已落地，但真实 OpenClaw live soak 进一步证明当前 decision transport 仍绑在宿主 runtime seam 上；当前优先路线因此改为“插件内自托管 memory + context decision overlay”，而不是直接修改 OpenClaw
...
| 时间层级 | 重点 | 退出信号 |
| --- | --- | --- |
| 当前 | 继续收 `Stage 7` closeout，并保持 `Stage 9` 为 default-off / opt-in only：先回答“怎样让每轮 context 更轻、更快、且不伤回答质量” | unified scorecard、guarded seam、rollback boundary 和 operator summary 都已落地，但 OpenClaw gateway live soak 证明当前 decision transport 仍被宿主 runtime seam 卡住 |
| 下一步 | 优先实现插件内自托管 `memory + context decision overlay`，先把 working-set decision transport 从宿主 `subagent` seam 上解开，再继续收 `Stage 7` closeout；ordinary-conversation Docker steady-state 继续作为默认 hermetic A/B 基线 | unified scorecard、guarded seam、rollback boundary 和 operator summary 都已稳定，且剩余 blocker 已明确缩到 plugin-owned decision runner 与 targeted Docker follow-up |
