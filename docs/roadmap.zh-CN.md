...
- 当前解释：Stage 7 的 operator-facing scorecard、Stage 9 的 guarded opt-in seam，以及 Step 108 的 plugin-owned transport closeout 都已落地；当前主问题已经不再是宿主 seam，而是继续补齐 harder eval matrix，并保持 Stage 9 仍然是极窄 opt-in 面
...
| Stage 9：guarded smart-path promotion | `in_progress` | context 加载优化如果一直停在 shadow-only，就还不是用户收益 | bounded、guarded 的窄路径 experiment 有明确 rollback boundary、operator metrics 和 promotion gate，并继续保持 compat / compact 只是后台保底而不是默认日常路径；Step 108 已关闭，下一步先补 harder matrix，再决定 guarded opt-in 是否扩大 |
...
| 时间层级 | 重点 | 退出信号 |
| --- | --- | --- |
| 当前 | 继续收 `Stage 7 / Context Minor GC`，并保持 `Stage 9` 为 default-off / opt-in only：先把 harder eval matrix 和 promotion gate 补齐，而不是重新回到宿主 seam 讨论 | unified scorecard、guarded seam、rollback boundary 和 operator summary 都已落地，Step 108 也已经关闭；当前剩余缺口是更硬的 case matrix 与 guarded gate |
| 下一步 | 先完成 `104` 的 harder eval matrix，并用同一套 operator scorecard 复测 `Context Minor GC`；ordinary-conversation Docker steady-state 继续作为默认 hermetic A/B 基线 | 更难的 case class 也能稳定说明更轻的 context package 不伤回答质量，且 Stage 9 仍然保持极窄 opt-in |
