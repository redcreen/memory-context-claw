...
这块记录的是 `Context Minor GC` 这条主线目前已经落到什么程度，包括已完成的 Stage 6 runtime shadow integration 和后续 Stage 7 / 9 证据。

- 专项别名：`context-minor-gc`
- Stage 6 专项名称：`dialogue-working-set-shadow-runtime`
...
- 当前解释：runtime shadow integration 已经成为 `Context Minor GC` 的 durable measurement surface，但 active prompt mutation 继续显式延后
...
- Stage 7 / Step 108 closeout：插件内自托管 decision runner 已落地，hermetic gateway `5 / 5` captured，本机真实 service smoke `3 / 3` captured
- 当前解释：Stage 7 的 operator-facing scorecard、harder live matrix、Stage 9 的 guarded live A/B，以及 Step 108 的 plugin-owned transport closeout 都已落地；Stage 10 也已经收口，当前主线不再是“继续打开下一段阶段”，而是保持这些证据面持续为绿
...
- [generated/stage9-guarded-smart-path-closeout-2026-04-18.zh-CN.md](../reports/generated/stage9-guarded-smart-path-closeout-2026-04-18.zh-CN.md)
...
- [generated/stage7-step108-context-minor-gc-closeout-2026-04-18.zh-CN.md](../reports/generated/stage7-step108-context-minor-gc-closeout-2026-04-18.zh-CN.md)
- [generated/stage7-context-minor-gc-closeout-2026-04-18.zh-CN.md](../reports/generated/stage7-context-minor-gc-closeout-2026-04-18.zh-CN.md)
...
- 当前维护重点：
  - 保持 `Context Minor GC / context 加载优化` 的统一 scorecard 持续为绿
  - 保持 Docker 作为默认 hermetic A/B 面
  - 保持 Stage 9 `default-off` / opt-in only 边界
  - 保持 Stage 10 的最短接入路径和 shared-foundation proof 不回退
- 当前明确不做：
...
| --- | --- | --- | --- |
| 轻快 | fact-first assembly、runtime working-set shadow instrumentation、release-preflight、Docker hermetic eval | runtime shadow replay `16 / 16`、average reduction ratio `0.4368`、runtime answer A/B `5 / 5` vs `5 / 5`、ordinary-conversation Docker strict closeout `current=40 / 40`、`legacy=15 / 40`、`UMC-only=25`、`preCaseResetFailed=0` | 先完成 `Context Minor GC / context 加载优化`，把 context thickness / reduction / latency 收成硬门禁；保持 Docker 作为默认 hermetic A/B 面，并把更难 ordinary-conversation matrix 收成 targeted follow-up |
| 聪明 | realtime `memory_intent` ingestion、nightly self-learning、governed promotion / decay、working-set shadow path | ordinary-conversation host-live A/B：current `38 / 40`、legacy `21 / 40`、`18` 条 UMC-only wins；Docker strict closeout：current `40 / 40`、legacy `15 / 40`、`25` 条 UMC-only wins | 把 shadow-first 的 context decision 逐步推进到 bounded、guarded 的窄路径用户收益，并让 context 优化真正变成默认可感知收益 |
| 省心 | add / inspect / audit / repair / replay / rollback、canonical registry root、OpenClaw / Codex adapters、Codex / 多实例 shared-foundation proof | 已发版级 CLI 流程与回归保护验证栈、Stage 10 shortest-path proof | 持续保持 operator surface 可读、可 replay，并让 Codex / 多实例 shared proof 持续为绿 |
...
- `轻快`
  ...
  - 这条热路径主线现在统一叫 `Context Minor GC`；目标是不把 compat / compact 当作默认续命手段，而是通过持续的 context 管理让 prompt 厚度和延迟都留在可控区间
- `聪明`
...
- 已经比较稳：
  ...
  - context 优化已经成为正式主线
- 当前剩余缺口：
  - `轻快`：逐轮 context 优化还没有变成默认 active-path 收益
  - `聪明`：working-set 优化仍然只是极窄 guarded opt-in 收益，不是默认体验
  - `省心`：当前不是“缺 Stage 10 证据”，而是要继续保持这些 cross-host/operator 证据长期为绿

这意味着下一轮 priority order 应该是：

1. 保持 `轻快 / context 加载优化` 的 unified scorecard 持续为绿
2. 保持 `轻快 / install` 的 Stage 10 最短接入路径持续为绿
3. 保持 `聪明` 的 bounded、guarded smart path 继续停留在极窄 opt-in 面
4. 保持 `省心` 的 shared-foundation/operator 证据持续稳定
...
| --- | --- | --- | --- |
| Stage 7：context loading optimization closure | `completed` | 当前最大的产品缺口不是“功能有没有”，而是每轮 context 仍然偏厚，Stage 6 也还只是 shadow measurement；这条主线现在统一对外叫 `Context Minor GC` | unified scorecard 固定；harder replay / Docker / local evidence 能稳定说明更轻的 context package 不伤回答质量；日常长对话通常不再需要依赖 compat / compact 才能继续 |
| Stage 8：ordinary-conversation realtime-write latency closure | `completed` | Docker ordinary-conversation A/B 已经从 blanket timeout wall 收口到 strict closeout `40 / 40 vs 15 / 40` 的可信结果；`2/4 shard gateway-steady` 只保留为 fast watch | clean Docker path 已不再被 timeout 大面积主导，且 `preCaseResetFailed = 0` 证明它可作为常规 A/B 面 |
| Stage 9：guarded smart-path promotion | `completed` | context 加载优化如果一直停在 shadow-only，就还不是用户收益 | bounded guarded seam 的 live A/B 已成立，rollback / operator contract 已明确；feature 继续保持 `default-off` / opt-in only，不扩大到默认路径 |
| Stage 10：adoption simplification and shared-foundation proof | `completed` | 当前 install / bootstrap / verify 已有最短维护者路径，Codex / 多实例 shared proof 也已落地 | `npm run umc:stage10 -- --format markdown` 持续为绿，且 shared-foundation 证据不回退 |

## 当前 / 下一步 / 更后面

| 时间层级 | 重点 | 退出信号 |
| --- | --- | --- |
| 当前 | Stage 7、Stage 8、Stage 9、Stage 10 都已正式收口 | Docker hermetic baseline、Stage 9 `default-off` 边界、Stage 10 最短接入路径与 shared-foundation proof 都持续为绿 |
| 下一步 | 进入维护态，默认保持现有门禁与证据面稳定 | 没有新的阶段被动打开；只有在出现新的明确产品目标时才开启下一阶段 |
| 更后面 | 如有新的明确用户目标，再定义新的阶段 | 新目标、边界、operator contract 和验证面都先写清楚 |
...
2. 继续保持 Docker 作为默认 hermetic A/B 面，并让普通对话 strict baseline、Stage 7、Stage 9、Stage 10 证据保持一致。
3. 继续把 active prompt mutation 明确排除在默认路径外，直到 rollback boundary 与 operator metrics 足够清楚。
4. 把 runtime export artifacts 和 Docker hermetic eval 一起当成 replayable operator evidence surface。
5. 只有在新的明确产品目标出现时，才打开新的阶段。

恢复执行时：

- 主顺序看 [reference/unified-memory-core/development-plan.zh-CN.md](reference/unified-memory-core/development-plan.zh-CN.md) 的 Stage 10 closeout 与维护态说明
...
| [Stage 7：context loading optimization closure](reference/unified-memory-core/development-plan.zh-CN.md#stage-7-context-loading-optimization-closure) | completed | 先把逐轮 context 加载优化做成正式主线和正式门禁，而不是继续停在 shadow 结论 | Stage 6 | context optimization scorecard 稳定、harder replay / Docker / local evidence 对齐，且 rollout/rollback 边界清楚 |
| [Stage 8：ordinary-conversation realtime-write latency closure](reference/unified-memory-core/development-plan.zh-CN.md#stage-8-ordinary-conversation-realtime-write-latency-closure) | completed | 把 clean Docker path 从 blanket timeout 收口成可信的 ordinary-conversation strict A/B 面 | Stage 7 | ordinary-conversation hermetic strict closeout 已达到 `40 / 40 vs 15 / 40`，且 `preCaseResetFailed = 0` |
| [Stage 9：guarded smart-path promotion](reference/unified-memory-core/development-plan.zh-CN.md#stage-9-guarded-smart-path-promotion) | completed | 在不破坏回退边界的前提下，让 context 优化开始变成真实用户收益 | Stage 8 | bounded opt-in path 已有 live A/B、明确 rollback boundary，并继续保持 `default-off` / opt-in only |
| [Stage 10：adoption simplification and shared-foundation proof](reference/unified-memory-core/development-plan.zh-CN.md#stage-10-adoption-simplification-and-shared-foundation-proof) | completed | 把接入体验和跨宿主产品证据收口到与核心能力同等重要的层级 | Stage 9 | `npm run umc:stage10 -- --format markdown` 成为最短维护者路径，且 Codex / 多实例 shared proof 已公开可读 |
...
- 真实 OpenClaw live soak 已经证明：剩余 blocker 主要不是算法，而是当前 decision transport 仍依赖宿主 subagent / request-scope seam
- 这次复盘也明确了：最早方案阶段应先完成 integration-point preflight；接入点、调用顺序、输入输出能力不应等到 live soak 再确认
