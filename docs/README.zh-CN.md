# 文档首页

[English](README.md) | [中文](README.zh-CN.md)

## 从这里开始

- 快速了解项目：[../README.zh-CN.md](../README.zh-CN.md)
- 架构总览：[architecture.zh-CN.md](architecture.zh-CN.md)
- 模块地图：[module-map.zh-CN.md](module-map.zh-CN.md)
- 路线图总览：[roadmap.zh-CN.md](roadmap.zh-CN.md)
- 测试计划：[test-plan.zh-CN.md](test-plan.zh-CN.md)
- 发布说明：[../RELEASE.zh-CN.md](../RELEASE.zh-CN.md)
- ADR 索引：[adr/README.zh-CN.md](adr/README.zh-CN.md)

## 按目标阅读

| 目标 | 阅读这里 |
| --- | --- |
| 快速试用项目 | [../README.zh-CN.md](../README.zh-CN.md) |
| 理解稳定系统结构 | [architecture.zh-CN.md](architecture.zh-CN.md) |
| 看官方模块边界 | [module-map.zh-CN.md](module-map.zh-CN.md) |
| 看里程碑层面的下一步 | [roadmap.zh-CN.md](roadmap.zh-CN.md) |
| 了解验证和发布门禁 | [test-plan.zh-CN.md](test-plan.zh-CN.md) |
| 深入查看详细执行队列 | [unified-memory-core/development-plan.md](unified-memory-core/development-plan.md) |

## 深层参考文档栈

- 详细架构地图：[unified-memory-core/architecture/README.md](unified-memory-core/architecture/README.md)
- 详细路线图：[unified-memory-core/roadmaps/README.md](unified-memory-core/roadmaps/README.md)
- 详细蓝图：[unified-memory-core/blueprints/README.md](unified-memory-core/blueprints/README.md)
- 详细测试体系：[unified-memory-core/testing/README.md](unified-memory-core/testing/README.md)
- 部署拓扑：[unified-memory-core/deployment-topology.md](unified-memory-core/deployment-topology.md)
- 所有权地图：[unified-memory-core/ownership-map.md](unified-memory-core/ownership-map.md)

## 维护者恢复边界

当前执行状态不放在这里。

恢复工作时先看控制面：

1. [../.codex/status.md](../.codex/status.md)
2. [../.codex/module-dashboard.md](../.codex/module-dashboard.md)
3. `../.codex/modules/*.md`

之后再回到这里看稳定参考文档。
