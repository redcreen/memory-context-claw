# Self-Learning Workstream Roadmap Index

[English](self-learning-workstream.md) | [中文](self-learning-workstream.zh-CN.md)

这份文档把 `self-learning workstream` 正式挂进统一的模块 roadmap 目录。

请直接配合下面两份主文档使用：

- workstream roadmap：
  [../../workstreams/self-learning/roadmap.md](../../../workstreams/self-learning/roadmap.zh-CN.md)
- workstream architecture：
  [../../workstreams/self-learning/architecture.md](../../../workstreams/self-learning/architecture.zh-CN.md)

当前状态：

- reflection baseline、daily reflection 和 standalone learning surfaces 之上的 Stage 3 lifecycle baseline 已完成
- Stage 4 policy adaptation 与多消费者使用已完成
- 下一阶段是产品加固，而不是继续做 Stage 4 contract discovery
